...
niceHeader:
  ...
  Beschreibung: "Это демонстрация загрузки YAML из папки assets мода"
```
...
публичный класс ModBehaviour : ModBehaviourBase
{
    защищенное переопределение void ModEnabled()
    {
        ...
        if (Datei.Exists(ymlPfad))
        {
            ...
            var niceData = Deserialisierer. Десериализация<NiceData>(ymlContent);
...
публичный класс NiceData
{
    ...
    публичный класс NiceDateHeader
    {
        ...
        public string Beschreibung { get; набор; } = строка. Пустой;
    }
}
```
