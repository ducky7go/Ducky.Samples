...
<ItemGroup>
  <PackageReference Include="Ducky.Sdk" Version="x.y.z">
    <PrivateAssets>やる</PrivateAssets>
    <IncludeAssets>エジェクシオン; コンストラワール; ナティボ; アーカイブ・デ・コンテニド; アナリザドーレス; ビルド推移的</IncludeAssets>
  </PackageReference>
</ItemGroup>
```
...
ModBehaviour de clase pública : ModBehaviourBase
{
    ...
    {
        _buffId = Contrato.Buffs.RegistroBuff<DoNothingBuff>(バフ=>
        {
            愛好 家。 SetDisplayName(LK. UI です。 DoNothingBuffName)
    ...
    private void SceneLoader_onAfterSceneInitialize(SceneLoadingContext obj)
    {
        if (obj.sceneName == GameplayDataSettings.SceneManagement.BaseScene.Name)
        {
            ...
            if (メイン != null)
            {
                ...
                校長。 AddBuff(Contract.Buffs.CreateBuffInstance(_buffId));
            }
        }
    }
}
...
コメンタリオ：'assets/lkeys.json' y 'assets/keys.hash.txt' se utilizan para el posicionamiento en tiempo de ejecución y la verificación del embalaje, y no se recomienda la modificación manual. Para forzar una actualización, limpie y vuelva a compilar el proyecto.
