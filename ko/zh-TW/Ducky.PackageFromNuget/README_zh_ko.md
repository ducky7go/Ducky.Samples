...
\| [중국어 간체자] (../../Ducky.PackageFromNuget/README.md) | [영어] (../../en/Ducky.PackageFromNuget/README_en.md) | [도이치] (../../de/Ducky.PackageFromNuget/README_de.md) | [프랑세] (../../fr/Ducky.PackageFromNuget/README_fr.md) | [스페인어] (../../es-ES/Ducky.PackageFromNuget/README_es.md) | [일본어] (../../ja/Ducky.PackageFromNuget/README_ja.md) | [한국어] (../../ko/Ducky.PackageFromNuget/README_ko.md) | [포르투갈어] (../../pt-PT/Ducky.PackageFromNuget/README_pt.md) | [Русский] (../../ru/Ducky.PackageFromNuget/README_ru.md) | [중국어 번체자] (README_zh.md) |
...
이 예제에서는 종속성 관리, 리소스 파일 액세스 및 종속성 패키징 배포를 포함하여 Mod에서 타사 NuGet 패키지를 사용하는 방법을 보여 줍니다(YamlDotNet을 예로 사용).

시작하기 전에 프론트엔드 환경 요구 사항이 준비되었는지 확인합니다： [환경 준비](.. /docs/Prequirement.md)를 사용합니다.

## 1. 프로젝트 소개

이 디렉터리(['Ducky.PackageFromNuget/'](Ducky.PackageFromNuget/))는 모드의 기능을 확장하는 방법을 보여줍니다：
...
**샘플 시나리오：** YamlDotNet 라이브러리를 사용하여 Mod 리소스 폴더에서 YAML 프로필을 읽고 확인합니다.
...
- SDK는 'PrivateAssets'를 사용하여 출력에 패키징되지 않도록 합니다.
- 타사 라이브러리(YamlDotNet)는 Mod 출력 디렉터리에 자동으로 패키징됩니다
...
1. 프로젝트를 마우스 오른쪽 단추로 클릭→ NuGet 패키지 관리
2. "YamlDotNet" 검색
3. "설치"를 클릭합니다.
...
1. 게임 실행
2. 모드 관리 인터페이스로 들어가세요
3. "Ducky.PackageFromNuget" 찾기
4. 활성화를 클릭합니다.
5. 게임 로그를 보면 구문 분석된 YAML 내용을 볼 수 있어야 합니다
...
- ⚠️ 라이브러리가 .NET Standard 2.1과 호환되는지 확인합니다.
