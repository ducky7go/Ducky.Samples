...
\| [Chinês simplificado] (../../Ducky.PackageFromNuget/README.md) | [Português] (../../en/Ducky.PackageFromNuget/README_en.md) | [Alemão] (README_de.md) | [Français] (../../fr/Ducky.PackageFromNuget/README_fr.md) | [Español] (../../es-ES/Ducky.PackageFromNuget/README_es.md) | [Japonês] (../../ja/Ducky.PackageFromNuget/README_ja.md) | [한국어] (../../ko/Ducky.PackageFromNuget/README_ko.md) | [Português] (../../pt-PT/Ducky.PackageFromNuget/README_pt.md) | [Русский] (../../ru/Ducky.PackageFromNuget/README_ru.md) | [Chinês tradicional] (../../zh-TW/Ducky.PackageFromNuget/README_zh.md) |
...
- ✅ Einführung em NuGet-Pakete von Drittanbietern
- ✅ Análise von Daten mithilfe externer Bibliotheken (YAML)
...
### Método 1：Verwenden der CLI (empfohlen)
...
### Método 2：Manuelles Bearbeiten von .csproj

Em '<ItemGroup>" Hinzugefügt：

```xml
<PackageReference Include="YamlDotNet" Version="16.3.0" />
```

### Método 3：Verwenden von Visual Studio
...
```yaml
niceHeader:
  Título: "Schöne YAML-Konfiguration"
  descrição: "Dies ist eine Demonstration des Ladens von YAML aus dem Assets-Ordner eines Mods"
```
...
mit Ducky.Sdk.ModComportamentos;
mit YamlDotNet.Serialização;
mit YamlDotNet.Serialization.NamingConventions;

Nomensraum Ducky.PackageFromNuget;
...
öffentliche Klasse ModBehaviour : ModBehaviourBase
{
    geschützte Überschreibung void ModEnabled()
    {
        ...
        var dir = Pfad.GetDirectoryName(typeof(ModBehaviour). Montagem.Ort)!;
        var ymlPath = Pfad.Kombinieren(Dir, "nice.yml");

        se (Datei.Exists(ymlPfad))
        {
    ...
    {
        Bereinigen von Ressourcen (cai erforderlich)
    }
}
...
öffentliche Klasse NiceData
{
    ...
    {
        string pública Titel { get; Garnitur; } = Zeichenfolge. Leer;
        string pública Beschreibung { get; Garnitur; } = Zeichenfolge. Leer;
    }
}
```
...
```csharp
Método 1：Nach Montageort (empfohlen)
var modDir = Pfad.GetDirectoryName(typeof(ModBehaviour). Montagem.Ort)!;
```
...
- ✅ Kopieren Sie die Ressourcendatei em das Ausgabeverzeichnis
...
- ✅ Comunidade Nutzen Sie-Ressourcen
