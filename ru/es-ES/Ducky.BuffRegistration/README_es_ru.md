...
<ItemGroup>
  <PackageReference Include="Ducky.Sdk" Version="x.y.z">
    <PrivateAssets>Дело</PrivateAssets>
    <IncludeAssets>Ejecución; конструктор; nativo; Archivos de contenido; Analizadores; buildtransitive</IncludeAssets>
  </PackageReference>
</ItemGroup>
```
...
ModBehaviour de clase pública : ModBehaviourBase
{
    ...
    {
        _buffId = Contrato.Buffs.RegistroBuff<DoNothingBuff>(бафф =>
        {
            болельщик. SetDisplayName(ЛК. Пользовательского интерфейса. DoNothingBuffName)
    ...
    private void SceneLoader_onAfterSceneInitialize(SceneLoadingContext obj)
    {
        if (obj.sceneName == GameplayDataSettings.SceneManagement.BaseScene.Name)
        {
            ...
            if (main != null)
            {
                ...
                основной. AddBuff(Contract.Buffs.CreateBuffInstance(_buffId));
            }
        }
    }
}
...
Коментарио：'assets/lkeys.json' y 'assets/keys.hash.txt' se utilizan para el posicionamiento en tiempo de ejecución y la verificación del embalaje, y no se recomienda la modificación manual. Para forzar una actualización, limpie y vuelva a compilar el proyecto.
